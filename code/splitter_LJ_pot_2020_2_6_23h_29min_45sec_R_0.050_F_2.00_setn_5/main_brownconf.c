...
# endif
  ...
  if(fabs(F) <= 0.1) simparams.n = simlong*simparams.n;

  if(F <= -2*pow(10,4)) simparams.n = 0.5*simlong*simparams.n;


  plotpoints = (long long) simparams.n/50; 
  testab = plotpoints*1;
  ...
  do{	
	  ...
	  if((i > simparams.n-testab) && (i <= simparams.n-testab+1)){ 
		  muabbo = tcoeff.mu;
           ...
           */ 	
	  if(((i > simparams.n) && (i%testab == 0)) || (i%plotpoints == 0)){ 
