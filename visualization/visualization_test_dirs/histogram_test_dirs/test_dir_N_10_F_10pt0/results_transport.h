...
typedef struct TAG_HistogramParams{
	...
	double bin2d;
	/* counters that are increased with every particle that was detected for the 
	 * calculation of the Histograms. If one of this counters deviates from the
	 * total number of particles after the simulation, an erroneusly behavior 
	 * was found.
	 */
	unsigned int xcounter;
	unsigned int ycounter;
	unsigned int twodcounter;	
