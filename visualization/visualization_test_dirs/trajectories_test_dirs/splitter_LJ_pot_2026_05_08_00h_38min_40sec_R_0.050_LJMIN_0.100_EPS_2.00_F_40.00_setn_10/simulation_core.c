...
/**
 * Revert adaption position according to period boundary
 * conditions employed for simulation
 */
static void invert_sim_shift_pos_for_periodic_bc(double *x, int *shiftind){
    if(*shiftind < 0){
       *x -= L_CONF;
    }
    if(*shiftind > 0){
...
static void sim_perform_valid_step(const T_SimParams *SimParams,
    ...
    do {
        ...
        if (!PosValid) {
            //invert_sim_shift_pos_for_periodic_bc(&x, &shiftind);
            shiftind = 0;
            x = xo;
            y = yo;
            break;
