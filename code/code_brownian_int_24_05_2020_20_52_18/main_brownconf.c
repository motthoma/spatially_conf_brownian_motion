...
void calc_transpcoeffs(
  ...
  /*mean cubic position <x^3> */
  tcoeff.meanxqub = xgesqub/SimParams.N*SimParams.numtasks;
  /*thrid moment of poisition <x^3> - 3<x><x^2> + 2<x>^3 */
                             ...
                             int abbdeff)
{
	...
	*/
	...
	fprintf(outpmom, "%.6f\t %.6lf\t %.5Lf\n", t, tcoeff.meanspeed, tcoeff.mthree);
	fclose(outpmom);
}
 ...
 */
{
  FILE *outp;
  outp=fopen(fname ,"a");
  fprintf(outp, "\n\nAverage of all Threads:\n\nmsd = %.5Lf\t meanspeed = %.5lf\t mu = %.5lf\t deff = %.5lf\n\n", 
...
//  bool MPI_ON;
  ...
  fprintf(outpmom, "#time\t tcoeff.meanspeed\t  <x^3>-3<x^2><x>+2<x>^3\n");
  fclose(outpmom);
...
# endif
  ...
  ParameterFlag = check_parameter_consistency();
...
#  endif
  ...
  if(taskid == MASTER){
 	   print_positions(setn_per_task, positionx, positiony);
           ...
           print_resallthreads(msdall, meanspeedall, muall, deffall, meanxall, meanxsquall, mthreeall, &fname[0], &fnamemom[0]);
