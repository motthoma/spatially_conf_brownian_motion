 ...
 */
  ...
  int nx = (int) L/bin2d;
  int ny = (int) (2*MAX_HALF_WIDTH)/bin2d;
	  ...
	  if(((i > n) && (i%testab == 0)) || (i%plotpoints == 0)){  
		  ...
		  for(j = 0; j < setn_per_task; j++){
			  for(kset = 0; kset < setnumb; kset++){
				 ...
				 xges = xges + (positionx[j][kset] - xstart[j][kset] - totalshift*L);
				 xgessquare = xgessquare + powl((positionx[j][kset] - xstart[j][kset] - totalshift*L),2);
                                 xgesqub = xgesqub + powl((positionx[j][kset] - xstart[j][kset] - totalshift*L),3);
...
#  endif
  ...
  sprintf(fnamey, "Erwy_Histogram_F_%.3lf.dat", F);
  ...
  sprintf(fname2d, "Erwpos_Histogram2d_F_%.3lf.dat", F);
