...
/*	mpi_ind = atof(argv[3]);
  ...
  positionx = calloc(simparams.N/setnumb, sizeof(double));
  positiony = calloc(simparams.N/setnumb, sizeof(double));
  fintxarray = calloc(simparams.N/setnumb, sizeof(double));
  fintyarray = calloc(simparams.N/setnumb, sizeof(double));
  xstart = calloc(simparams.N/setnumb, sizeof(double));
  for(i = 0; i < simparams.N/setnumb; i++){
  ...
  long int **negshift;
  long int **posshift;
  negshift = calloc(simparams.N/setnumb, sizeof(long int));
  posshift = calloc(simparams.N/setnumb, sizeof(long int));
  for(i = 0; i < simparams.N/setnumb; i++){
...
# endif
   ...
   */ 
  setn_per_task = (int) simparams.N/(setnumb*numtasks);
  ...
  if(taskid == MASTER){
	  ...
	  if(simparams.N % (numtasks*setnumb) != 0){ 
